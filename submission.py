...
class TextCNN(object):
    ...
    def __init__(self, sequence_length, embedding_size, word_vector, filter_sizes, num_filters):
        ...
        with tf.name_scope('fully_connnected'):
            ...
            self.full_connect2 = fully_connected(self.full_connect1_dropout, num_outputs=200, activation_fn=None)

        l2_loss = tf.constant(0.0)
        # get output
        with tf.name_scope('output'):
            W = tf.get_variable('output_W', shape=[200, 5],
...
def save_data(sess, test_x, model):
    ...
    for i in range(data_size // 500):
        result.append(
            sess.run(model.output, feed_dict={model.input_x: test_x[i * 500:(i + 1) * 500], model.keep_prob: 1.0}))
...
def eval_train_data(sess, train_x_all, model):
    data_size = len(train_x_all)
    result = []
    for i in range(data_size // 500):
        result.append(sess.run(model.output, feed_dict={model.input_x: train_x_all[i * 500:(i + 1) * 500], model.keep_prob: 1.0}))
    result.append(sess.run(model.output, feed_dict={model.input_x: train_x_all[(i + 1) * 500:], model.keep_prob: 1.0}))
    result = np.concatenate(result, axis=0)

    train_data = pd.read_table(train_path, sep='\t')
    train_data['predictions'] = result.reshape(-1).tolist()
    print((train_data['Sentiment'] == train_data['predictions']).mean())
    train_data.to_csv("result_train.csv", index=False)
...
def train(train_x, train_y, word_processor, test_x, train_x_all):
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            batches_train = batches_generate(list(data_train), epoch_size=1, batch_size=1, shuffle=True)

            total = (len(train_x) // 64 + 1) * 10
            # training
            # for i, batch in enumerate(batches_train):
            #    batch_x, batch_y = zip(*batch)
            #    loss, _, accuracy = sess.run([cnn.loss, train_op, cnn.accuracy], feed_dict={cnn.input_x: batch_x, cnn.input_y: batch_y, cnn.keep_prob: 0.7})
            ...
            eval_train_data(sess, train_x_all, cnn)
